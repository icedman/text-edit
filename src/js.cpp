...
JS::JS() { js_instance = this; }

JS *JS::instance() { return js_instance; }
...
void JS::initialize() {
  ...
  // JS_SetPropertyStr(ctx, global_obj, "log", JS_NewCFunction(ctx, js_log,
  // "log", 1));
  ...
  ret = JS_Eval(ctx, init_script.c_str(), init_script.length(), "<input>",
                JS_EVAL_TYPE_MODULE);
  ...
  run_file("/home/iceman/Developer/Projects/text_edit_superstring/runtime/js/"
           "require.js");
  run_file("/home/iceman/Developer/Projects/text_edit_superstring/runtime/js/"
           "polyfill.js");
  run_file("/home/iceman/Developer/Projects/text_edit_superstring/runtime/"
           "tests/index.js");
#endif
}
...
#if ENABLE_JS
  JSValue ret;
  ret = JS_Eval(ctx, script.c_str(), script.length(), path.c_str(),
                JS_EVAL_TYPE_GLOBAL);
  if (JS_IsException(ret)) {
    js_std_dump_error(ctx);
    JS_ResetUncatchableError(ctx);
